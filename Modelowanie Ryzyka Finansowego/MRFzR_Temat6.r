...
# W przypadku testow warunkow skrajnych zazwyczaj wykorzystujemy jak najdłuższą probe
T     <- length(r)
N     <- T                               
r     <- tail(r,N)                       
R     <- coredata(r)
...
# rolowana minimum w ciągu następnych w_length dni
...
# data poczatkowa i koĹ„cowa spadku
...
RF2 = sum(c(0,0,0,0,-0.5 )*A)   #wzroście cen złota o 20%
# procentowa reakcja na wzrost krzywej dochodowosci na calej dlugosci o 100 pb.
RF3 = sum(c(1.5,1.5,0,0,0 )*A)   #spadek cen akcji na rynkach rozwijajacych się i rozwiniętych o 15%
# procentowa reakcja na deprecjacje kursu o 1%
RF4 = sum(c(0,0,5,3,0)*A)   #przesunięciu krzywej dochodowości o 50 pb w dół
# procentowa reakcja na wzrost spreadu kredytowego o 100 pb.
RF5 = sum(c(0,0.5,0,0,0.5)*A)   # odpływie kapitału z rynków wschodzących i deprecjacji ich kursów o 10%
